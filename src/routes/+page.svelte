<script lang="ts">
	...
	function onFilterChange(event: CustomEvent<{ genre: string; year: string; search: string }>) {
		...
		filtered = data.popularMovies.filter((movie) => {
			const matchGenre = !genre || movie.genre_ids.includes(Number(genre));
			const matchYear = !year || (movie.release_date && movie.release_date.startsWith(year));
			const matchSearch = !search || movie.title.toLowerCase().includes(search.toLowerCase());
